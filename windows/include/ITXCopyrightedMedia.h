...
class ITXCopyrightedMedia {
   public:
     ...
     * 预加载音速达音乐数据
     *
     * @param musicId 音速达歌曲Id
     * @param bitrateDefinition 码率，传空为音频默认码率，一般格式为：audio/mi:
     ...
     * 32,audio/lo: 64,audio/hi: 128
     */
    virtual void preloadClimaxSegment(const char* musicId, const char* bitrateDefinition, const char* playToken) = 0;
     ...
     * 32,audio/lo: 64,audio/hi: 128
     */
    virtual void preloadPitchAndLyricFile(const char* musicId, const char* bitrateDefinition, const char* playToken) = 0;
     ...
     * @param bitrateDefinition 码率，传nil为音频默认码率，一般格式为：audio/mi:
     * 32,audio/lo: 64,audio/hi: 128
     * @param bgmType 高潮片段的类型：4：原唱高潮片段 5：伴奏高潮片段
     * @param TXClimaxTimeRange 高潮片段相当于原唱文件的相对时间戳
     * 一般下载下来的高潮片段都是比较短的。如果用高潮片段进行打分。需要将歌词文件和打分文件 seek 到相应的位置。
     * 例如一个原唱文件的高潮开始的位置在 1800ms 到 2800ms 的位置。 那么此时这两个值返回的就是 1800 和 2800。
     * 此时在打分开始的时候，可以将打分和歌词文件 seek 到 1800 的位置。当播放到 2800 的位置时结束打分即可。
     */
    virtual bool getClimaxSegmentTimeRange(const char* music_id, int bgmType, const char* bitrateDefinition, TXClimaxTimeRange* timeRange) = 0;
     ...
     * @param bitrateDefinition 码率，传nil为改音频默认码率
     */
    virtual void clearMusicCache(const char* musicId, const char* bitrateDefinition) = 0;
...
class ITXCopyrightedSongScore {
   public:
     ...
     * 通常用于升降音调唱歌，例如用户要提高一个半音唱歌，此时shiftKey = 1。
