/**
 * Copyright (c) 2024 Tencent. All rights reserved.
 * Module:   本地合图模块（当前版本仅支持 Windows/Mac 平台，虚拟背景、绿幕、手机投屏目前仅 windows 支持）。
 * Function: 用于对本地媒体源进行合图。
 */
#if __APPLE__
#include <TargetConditionals.h>
#endif

#if defined(_WIN32) || defined(TARGET_OS_MAC)

#ifndef TRTC_CPP_ITXLOCALMEDIATRANSCODING_H_
#define TRTC_CPP_ITXLOCALMEDIATRANSCODING_H_

#include "ITRTCCloud.h"
#include "TRTCTypeDef.h"
#include "TXLiteAVBase.h"

namespace liteav {
class ITXLocalMediaTranscoding;
}

/**
 * 利用 C 函数获取 ITXLocalMediaTranscoding 实例。
 *
 * 你可以按照下列方式创建和销毁 ITXLocalMediaTranscoding Instance
 *
 * <pre>
 * ITXLocalMediaTranscoding *instance = createTXLocalMediaTranscoding();
 * if(instance) {
 *     instance->startTranscoding(xxxx);
 * }
 * destroyTXLocalMediaTranscoding(instance);
 * instance = nullptr;
 * </pre>
 */
extern "C" {
LITEAV_API liteav::ITXLocalMediaTranscoding* createTXLocalMediaTranscoding();

LITEAV_API void destroyTXLocalMediaTranscoding(liteav::ITXLocalMediaTranscoding* instance);
}

namespace liteav {

/**
 * 媒体源类型。
 */
enum LocalMediaTranscodingSourceType {

    /// 摄像头媒体类型。
    MediaSourceCamera = 0,

    /// 屏幕媒体类型。
    MediaSourceScreen = 1,

    /// 图片媒体类型，目前只支持 BMP、JPG、PNG、GIF 四种格式。
    MediaSourceImage = 2,

    /// 预留字段，当前版本不支持远端用户视频源混流转码。
    MediaSourceRemoteVideo = 3,

    /// 手机投屏类型。
    ///@note 当前仅 Windows 端支持。
    MediaSourcePhoneMirror = 4,

    /// 在线视频源。目前支持 RTMP、HTTP-FLV、在线视频文件
    MediaSourceOnlineVideo = 5,

    /// 视频文件源。目前支持 Mp4 文件
    MediaSourceVideoFile = 6,

};

/**
 * 媒体源
 */
struct LocalMediaTranscodingSource {
    /// 【字段含义】媒体类型。
    LocalMediaTranscodingSourceType sourceType = LocalMediaTranscodingSourceType::MediaSourceCamera;

    /// 【字段含义】摄像头 ID。
    /// 当 sourceType 为 MediaSourceCamera 时此字段有效，您可以使用该字段指定摄像头的设备 ID，
    /// 摄像头的设备 ID 可以通过 TXDeviceManager 中的接口获取。
    const char* cameraDeviceId = nullptr;

    /// 【字段含义】屏幕或窗口 ID。
    /// 当 sourceType 为 MediaSourceScreen 时此字段有效，您可以使用该字段指定整个屏幕或者某个窗口的窗口 ID。
    /// 您可以通过 TRTCCloud 中的 getScreenCaptureSources 接口获取可以被分享的窗口 ID。
    TXView screenSourceId = nullptr;

    /// 【字段含义】图像的地址。
    /// 当 sourceType 为 MediaSourceImage 时此字段有效，您可以使用该字段指定图片的文件路径。
    /// 目前版本只支持 BMP、JPG、PNG、GIF 四种图像格式。
    const char* imagePath = nullptr;

    /// 【字段含义】远端视频流的用户 ID。
    /// 当 sourceType 为 MediaSourceRemoteVideo 时此字段有效，不过当前版本的 SDK 还不支持这个能力。
    const char* userId = nullptr;

    /// 【字段含义】手机投屏 ID。
    /// 当 sourceType 为 MediaSourcePhoneMirror 时此字段有效，您可以使用该字段指定投屏ID。
    /// 当前仅 Windows 端支持。
    const char* phoneMirrorSourceId = nullptr;

    /// 【字段含义】在线视频的 url。
    /// 当 sourceType 为 MediaSourceOnlineVideo 时此字段有效，您可以使用该字段指定在线视频的 url。
    const char* onlineVideoUrl = nullptr;

    /// 【字段含义】本地视频文件的路径。
    /// 当 sourceType 为 MediaSourceVideoFile 时此字段有效，您可以使用该字段指定视频文件的路径。
    const char* videoFilePath = nullptr;

    /// 【字段含义】指定该路画面的坐标区域（单位：像素）。
    RECT rect = RECT{0};

    /// 【字段含义】指定该路画面的层级（取值范围：0 - 15，不可重复）。
    int zOrder = 0;

    /// 【字段含义】指定该路数据源是否旋转、镜像和填充模式，详情请参见 {@link TRTCRenderParams}。
    const TRTCRenderParams* renderParams = nullptr;

    /// 【字段含义】该路画面是否处于选中状态。
    bool isSelected = false;
};

/**
 * 本地媒体流混流转码配置参数。
 */
struct LocalMediaTranscodingParams {
    /// 【字段含义】指定转码流中的每一路输入媒体源配置的信息。
    /// 该字段是一个 LocalMediaTranscodingSource 类型的数组，数组中的每一个元素都用来代表每一路输入媒体源的信息。
    /// 【特别说明】媒体源信息不支持留空，否则 ITXLocalTranscodingCallback::onTranscodingStarted 会报错。
    LocalMediaTranscodingSource* inputSourceList = nullptr;

    /// 【字段含义】数组 inputSourceList 的大小。
    unsigned int inputSourceListSize = 0;

    /// 【字段含义】转码流的视频编码参数。
    TRTCVideoEncParam videoEncoderParams;

    /// 【字段含义】指定混合画面的底色。
    /// 【推荐取值】默认值：0x000000 代表黑色。格式为十六进制数字，例如：“0x61B9F1” 代表 RGB 分别为（97、158、241）。
    int canvasColor = 0x000000;

    /// 【字段含义】指定输入源被选中时的边框颜色。
    /// 【推荐取值】默认值：0xFFFF00 代表黄色。格式为十六进制数字，例如：“0x61B9F1” 代表 RGB 分别为（97、158、241）。
    int inputSourceBorderColor = 0xFFFF00;
};

/**
 * 本地媒体流混流转码错误码。
 */
enum LocalMediaTranscodingError {

    /// 调用成功。
    Success = 0,

    /// 通用错误码。
    Error = -1,

    /// 参数非法。
    InvalidParams = -2,

    /// 某个数据源混流异常，LocalMediaTranscodingSource 某条数据源超过 30s 没数据抛此异常。
    NotFoundSource = -3,

    /// 图片源加载失败返回此错误码。
    ImageSourceLoadFailed = -4,

    /// 用户未授权当前应用使用摄像头。
    CameraNotAuthorized = -5,

    /// 摄像头正在被占用中，可尝试打开其他摄像头。
    CameraIsOccupied = -6,

    /// 摄像头无法连接。
    CameraDisconnected = -7,

    /// 不支持的协议类型。
    UnsupportedOnlineVideoProtocol = -8,

    /// 不支持的文件格式。
    UnsupportedLocalVideoFileFormat = -9,

    /// 无法连接服务器。
    OnlineVideoConnectFailed = -10,

    /// 连接断开。
    OnlineVideoConnectionLost = -11,

    /// 不支持 HEVC 解码。
    NoAvailableHevcDecoder = -12,

    /// 视频文件不存在。
    VideoFileNotExist = -13

};

/**
 * 摄像头源美颜参数
 */
struct TXCameraBeautyParam {
    /// 【字段含义】美颜风格。会整体影响下面磨皮、美白、锐化、红润的效果。2-优图美颜仅在 Mac 端生效，Windows 端设置该美颜风格将显示原画面。
    /// 【取值范围】0-平滑美颜，1-自然美颜，2-优图美颜，默认值：0。
    TXBeautyStyle beautyStyle = TXBeautyStyle::TXBeautyStyleSmooth;

    /// 【字段含义】磨皮程度。
    /// 【取值范围】[0.0, 1.0]，默认值：0.0。
    float skinSmoothingLevel = 0.0f;

    /// 【字段含义】美白程度。
    /// 【取值范围】[0.0, 1.0]，默认值：0.0。
    float whitenessLevel = 0.0f;

    /// 【字段含义】锐化程度。
    /// 【取值范围】[0.0, 1.0]，默认值：0.0。
    float sharpenLevel = 0.0f;

    /// 【字段含义】红润程度。
    /// 【取值范围】[0.0, 1.0]，默认值：0.0。
    float ruddyLevel = 0.0f;
};

/**
 * 手机投屏源配置参数
 *
 * @note 当前仅 Windows 端支持。
 */
struct TXPhoneMirrorParam {
    /// 【字段含义】投屏发送端系统类型。
    /// 【取值范围】-1-未知系统类型, 0-Android, 1-iOS。
    int32_t platformType = -1;

    /// 【字段含义】投屏连接方式：
    /// 【取值范围】-1-未知连接方式, 0-有线投屏；1-无线投屏。目前仅支持有线投屏。
    int32_t connectType = -1;

    /// 【字段含义】投屏发送端设备Id, 该字段可为空。
    const char* deviceId = nullptr;

    /// 【字段含义】投屏发送端设备名称, 该字段可为空。
    const char* deviceName = nullptr;

    /// 【字段含义】占位图路径，设备断联时显示的图片，该字段可为空。
    const char* placeholderImagePath = nullptr;

    /// 【字段含义】发送端屏幕采集帧率。
    /// 【推荐取值】默认值：60。推荐取值范围：(0, 60]
    uint32_t frameRate = 60;

    /// 【字段含义】视频投屏码率,单位：kbps。
    /// 【推荐取值】默认值：10000。
    uint32_t bitrateKbps = 10000;
};

/**
 * 在线视频播放参数
 */
struct OnlineVideoParam {
    /// 【字段含义】播放器网络缓存大小，取值范围：[0, 16*1024]，当设置成0时会自动设置合适的大小，单位：KB
    int networkCacheSizeKB = 1024;

    /// 【字段含义】设置该在线视频播放的音量大小
    int playoutVolume = 100;
};

/**
 * 视频文件播放参数
 */
struct VideoFileParam {
    /// 【字段含义】设置该视频文件播放的音量大小
    int playoutVolume = 100;
};

class ITXVideoFrameProcessCallback {
   public:
    virtual ~ITXVideoFrameProcessCallback() {
    }

    /**
     * 自定义图像处理回调（对接第三方美颜组件）。
     *
     * 如果您选购了第三方美颜组件，就需要在 ITXLocalMediaTranscoding 中设置第三方美颜回调，之后
     * ITXLocalMediaTranscoding 就会将原本要进行预处理的视频帧通过此回调接口抛送给您。之后您就可以将
     * ITXLocalMediaTranscoding 抛出的视频帧交给第三方美颜组件进行图像处理，由于抛出的数据是可读且可写的，因此第三方美颜的处理结果也可以同步给
     * ITXLocalMediaTranscoding 进行后续的编码和发送。
     *
     * 情况一：美颜组件自身会产生新的纹理。
     * 如果您使用的美颜组件会在处理图像的过程中产生一帧全新的纹理（用于承载处理后的图像），那请您在回调函数中将
     * processedVideoFrame.textureId 设置为新纹理的 ID：
     *
     * <pre>
     * int onProcessCameraVideo(char* cameraDeviceId,TRTCVideoFrame*
     * originalVideoFrame,TRTCVideoFrame* processedVideoFrame) {
     *     processedVideoFrame->textureId =
     *     mFURenderer.onDrawFrameSingleImput(originalVideoFrame->textureId);
     *     return 0;
     * }
     * </pre>
     *
     * 情况二：美颜组件需要您提供目标纹理。
     * 如果您使用的第三方美颜模块并不生成新的纹理，而是需要您设置给该模块一个输入纹理和一个输出纹理，则可以考虑如下方案：
     *
     * <pre>
     * int onProcessCameraVideo(char* cameraDeviceId,TRTCVideoFrame*
     * originalVideoFrame,TRTCVideoFrame* processedVideoFrame) {
     *     thirdparty_process(originalVideoFrame->textureId,originalVideoFrame->width,originalVideoFrame->height,
     *     processedVideoFrame->textureId);
     *     return 0;
     * }
     * </pre>
     *
     * @param originalVideoFrame 用于承载 TRTC 采集到的摄像头画面。
     * @param processedVideoFrame 用于接收第三方美颜处理过的视频画面。
     * @return - 0：   成功。
     *         - 其他： 错误。
     * @note 目前仅支持 OpenGL 纹理方案（PC 仅支持 TRTCVideoBufferType_Buffer 格式）。
     */
    virtual int onProcessVideoFrame(const char* cameraDeviceId, const TRTCVideoFrame* originalVideoFrame, TRTCVideoFrame* processedVideoFrame) = 0;
};

class ITXVideoFrameRenderCallback {
   public:
    virtual ~ITXVideoFrameRenderCallback() {
    }

    /**
     * 自定义图像渲染回调（用于自定义显示混合后的画面）。
     *
     * @param mixedVideoFrame 经过本地混合后的视频图像。
     */
    virtual int onRenderMixedFrame(const TRTCVideoFrame* mixedVideoFrame) = 0;
};

class ILocalMediaTranscodingCallback {
   public:
    virtual ~ILocalMediaTranscodingCallback() {
    }

    /**
     * 本地混流转码开启的事件回调。
     *
     * 当您调用 {@link startTranscoding} 时启动本地混流转码时，是否启动成功会通过此回调同步给您。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg 错误信息。
     */
    virtual void onTranscodingStarted(LocalMediaTranscodingError errCode, const char* errMsg) = 0;

    /**
     * 本地混流转码停止的事件回调。
     *
     * 当您通过 {@link stopTranscoding} 停止本地混流转码时，SDK 便会抛出此事件回调。
     * @param reason 停止原因
     *
     *    - 0：用户主动停止。
     *    - 1：被动停止，有可能采集源失效了。
     * @param reasonMsg 停止原因描述信息。
     */
    virtual void onTranscodingStopped(int reason, const char* reasonMsg) = 0;

    /**
     * 本地摄像头开启的事件回调。
     *
     * 当您调用 {@link startCameraSource} 打开摄像头时，是否启动成功会通过此回调同步给您。
     * @param deviceId 摄像头设备 Id。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg 错误信息。
     */
    virtual void onCameraSourceStarted(const char* deviceId, LocalMediaTranscodingError errCode, const char* errMsg) = 0;

    /**
     * 本地摄像头停止的事件回调。
     *
     * @param deviceId 摄像头设备 Id。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param reasonMsg 停止原因描述信息。
     */
    virtual void onCameraSourceStopped(const char* deviceId, LocalMediaTranscodingError errCode, const char* reasonMsg) = 0;

    /**
     * 图片混流开启的事件回调。
     *
     * 当您调用 {@link addImageSource} 打开图片采集时，是否启动成功会通过此回调同步给您。
     * @param imagePath 图片路径。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg 错误信息。
     */
    virtual void onImageSourceStarted(const char* imagePath, LocalMediaTranscodingError errCode, const char* errMsg) = 0;

    /**
     * 图片混流停止的事件回调。
     *
     * @param imagePath 图片路径。
     */
    virtual void onImageSourceStopped(const char* imagePath) = 0;

    /**
     * 屏幕分享开启的事件回调。
     *
     * 当您通过 {@link startScreenSource} 等相关接口启动屏幕分享时，SDK 便会抛出此事件回调。
     * @param sourceId 屏幕或者窗口 Id。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError} 。
     * @param errMsg 错误信息。
     */
    virtual void onScreenSourceStarted(TXView sourceId, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 屏幕分享暂停的事件回调。
     *
     * @param sourceId 屏幕或者窗口 Id。
     * @param reason 原因
     *
     *    - 0：表示用户主动暂停。
     *    - 1：表示屏幕分享窗口不可见导致的暂停（仅 Mac）。
     *    - 2：表示屏幕分享窗口被最小化导致的暂停（仅 Windows）。
     *    - 3：表示屏幕分享窗口被隐藏导致的暂停（仅 Windows）。
     */
    virtual void onScreenSourcePaused(TXView sourceId, int reason) {
    }

    /**
     * 屏幕分享恢复的事件回调。
     *
     * @param sourceId 屏幕或者窗口 Id。
     * @param reason 恢复原因
     *
     *    - 0：表示用户主动恢复。
     *    - 1：表示屏幕分享窗口可见导致的恢复（仅 Mac）。
     *    - 2：表示屏幕分享窗口从取消最小化导致的恢复（仅 Windows）。
     *    - 3：表示屏幕分享窗口从取消隐藏导致的恢复（仅 Windows）。
     */
    virtual void onScreenSourceResumed(TXView sourceId, int reason) {
    }

    /**
     * 屏幕分享停止的事件回调。
     *
     * 当您通过 {@link stopScreenSource} 停止屏幕分享时，SDK 便会抛出此事件回调。
     *
     * @param sourceId 屏幕或者窗口 Id。
     * @param reason 停止原因
     *
     *    - 0：表示用户主动停止。
     *    - 1：表示屏幕分享窗口关闭导致的停止。
     *    - 2：表示屏幕分享显示器状态变更（如接口被拔出、投影模式变更等）导致的停止。
     */
    virtual void onScreenSourceStopped(TXView sourceId, int reason) {
    }

    /**
     * 手机投屏开启的事件回调。
     *
     * 当您通过 {@link startPhoneMirrorSource} 开启投屏时，SDK 便会抛出此事件回调。
     *
     * @param phoneMirrorSourceId 手机投屏Id。
     * @param errCode             错误码，详情请参见 {@link LocalMediaTranscodingError} 。
     * @param errMsg              提示信息。
     * @note 当前仅 Windows 端支持。
     */
    virtual void onPhoneMirrorSourceStarted(const char* phoneMirrorSourceId, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 手机投屏停止的事件回调。
     *
     * @param phoneMirrorSourceId 手机投屏Id。
     * @param errCode             错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param reasonMsg           停止原因描述信息。
     * @note 当前仅 Windows 端支持。
     */
    virtual void onPhoneMirrorSourceStopped(const char* phoneMirrorSourceId, LocalMediaTranscodingError errCode, const char* reasonMsg) {
    }

    /**
     * 手机投屏设备状态变更的事件回调。
     *
     * 当您调用 {@link startPhoneMirrorSource} 后，每当投屏设备状态变更时 SDK 便会抛出此事件回调。
     *
     * @param param 变更的设备信息，详情请参见 {@link TXPhoneMirrorParam}。
     * @param state 设备当前状态。0-added，1-connected，2-disconnected，3-removed。
     * @note 当前仅 Windows 端支持。
     */
    virtual void onPhoneMirrorSourceChanged(const TXPhoneMirrorParam& param, int state) {
    }

    /**
     * 在线视频正在加载中。
     * 在线视频播放中途出现加载的话，SDK 便会抛出此事件回调，此时音频、画面可能会卡住。
     * @param url 播放地址。
     */
    virtual void onOnlineVideoLoading(const char* url) {
    }

    /**
     * 在线视频开始播放的事件回调。
     * 当您通过 {@link addOnlineVideoSource} 播放直播流时，SDK 便会抛出此事件回调。
     * @param url     播放地址。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg  提示信息。
     */
    virtual void onOnlineVideoStarted(const char* url, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 在线视频播放停止的事件回调。
     * @param url     播放地址。
     * @param errCode 错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg  停止原因描述信息。
     */
    virtual void onOnlineVideoStopped(const char* url, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 视频文件开始播放的事件回调。
     * 当您通过 {@link addVideoFileSource} 播放视频文件时，SDK 便会抛出此事件回调。
     * @param videoFilePath 视频文件地址。
     * @param errCode       错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg        提示信息。
     */
    virtual void onVideoFileStarted(const char* videoFilePath, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 视频文件播放停止的事件回调。
     * @param videoFilePath 视频文件地址。
     * @param errCode       错误码，详情请参见 {@link LocalMediaTranscodingError}。
     * @param errMsg        提示信息。
     */
    virtual void onVideoFileStopped(const char* videoFilePath, LocalMediaTranscodingError errCode, const char* errMsg) {
    }

    /**
     * 视频文件播放进度的事件回调。
     *
     * 当您通过 {@link addVideoFileSource} 播放视频文件，SDK 会每隔 500ms 抛出此事件回调。
     * @param videoFilePath 视频文件地址。
     * @param currentTimeMs 当前播放时间，单位为毫秒。
     * @param durationMs    视频文件总时长，单位为毫秒。
     */
    virtual void onVideoFilePlayProgress(const char* videoFilePath, const int64_t currentTimeMs, const int64_t durationMs) {
    }

    /**
     * 媒体源画面大小发生变化的事件回调。
     *
     * 当输入媒体源的画面大小发生变化时，会通过该接口返回该输入源最新的尺寸，您可以根据该尺寸来动态调整画面的比例。
     * @param mediaSource 字段代表该路输入媒体源信息。
     * @param newSize 字段代表该路输入媒体源最新的画面大小。
     * @note 目前媒体源类型仅支持窗口采集。
     */
    virtual void onMediaSourceSizeChanged(const LocalMediaTranscodingSource& mediaSource, const SIZE& newSize) {
    }
};

class ITXLocalMediaTranscoding {
   public:
    /**
     * 设置 TXLocalMediaTranscoding 事件回调。
     *
     * 您可以通过 {@link ILocalMediaTranscodingCallback} 获得来自 SDK 的各类事件通知（例如：错误码，警告码，接口调用状态等）。
     * @param callback 回调实例
     */
    virtual void setTranscodingCallback(ILocalMediaTranscodingCallback* callback) = 0;

    /**
     * 设置 ITXLocalMediaTranscoding 视频自定义预处理数据回调。
     *
     * 您可以通过 {@link setVideoFrameProcessCallback} 获得来自 ITXVideoFrameProcessCallback 摄像头自定义预处理数据。
     * @param pixelFormat 指定回调的像素格式，目前仅支持 TRTCVideoPixelFormat_I420 || TRTCVideoPixelFormat_BGRA32 || TRTCVideoPixelFormat_RGBA32 格式。
     * @param bufferType  指定视频数据结构类型，目前仅支持 TRTCVideoBufferType_Buffer。
     * @param callback 回调实例
     */
    virtual void setVideoFrameProcessCallback(TRTCVideoPixelFormat pixelFormat, TRTCVideoBufferType bufferType, ITXVideoFrameProcessCallback* callback) = 0;

    /**
     * 设置 ITXLocalMediaTranscoding 视频自定义渲染数据回调。
     *
     * 您可以通过 {@link setVideoFrameRenderCallback} 获得来自 ITXVideoFrameRenderCallback 业务需要的自定义渲染数据。
     * 您可以通过调用 setVideoFrameRenderCallback(TRTCVideoPixelFormat_Unknown, TRTCVideoBufferType_Unknown, nullptr) 停止回调：
     *
     *   - iOS、Mac、Windows 平台目前仅支持回调 {@link TRTCVideoPixelFormat_I420} 或 {@link TRTCVideoPixelFormat_BGRA32} 像素格式的视频帧。
     *   - Android 平台目前仅支持传入 {@link TRTCVideoPixelFormat_I420}, {@linkTRTCVideoPixelFormat_RGBA32} 或 {@link TRTCVideoPixelFormat_Texture_2D} 像素格式的视频帧。
     * @param pixelFormat 指定回调的像素格式
     * @param bufferType  指定视频数据结构类型，目前只支持 {@link TRTCVideoBufferType_Buffer} 内存格式数据。
     * @param callback 回调实例
     */
    virtual void setVideoFrameRenderCallback(TRTCVideoPixelFormat pixelFormat, TRTCVideoBufferType bufferType, ITXVideoFrameRenderCallback* callback) = 0;

    /**
     * 启动本地混流转码。
     *
     * @param streamType 指定使用 TRTC 的 TRTCVideoStreamTypeBig 来推流，还是 TRTCVideoStreamTypeSub 来推流。
     * @param params 指定本地混流转码的参数，详情请参见 {@link LocalMediaTranscodingParams}。
     */
    virtual void startTranscoding(TRTCVideoStreamType streamType, const LocalMediaTranscodingParams& params) = 0;

    /**
     * 更新本地混流转码参数。
     *
     * @param params 指定本地混流转码的参数，详情请参见  {@link LocalMediaTranscodingParams}。
     */
    virtual void updateTranscodingParams(const LocalMediaTranscodingParams& params) = 0;

    /**
     * 停止本地混流转码。
     */
    virtual void stopTranscoding() = 0;

    /**
     * 启动摄像头采集。
     *
     * @param cameraDeviceId 摄像头设备 Id。
     */
    virtual void startCameraSource(const char* cameraDeviceId) = 0;

    /**
     * 停止摄像头采集。
     *
     * @param cameraDeviceId 摄像头设备 Id。
     */
    virtual void stopCameraSource(const char* cameraDeviceId) = 0;

    /**
     * 设置摄像头采集参数。
     *
     * @param cameraDeviceId 摄像头设备 Id。
     * @param cameraParam 指定摄像头采集分辨率和帧率，详情请参见  {@link TXCameraCaptureParam}。
     */
    virtual void setCameraCaptureParam(const char* cameraDeviceId, const TXCameraCaptureParam* cameraParam) = 0;

    /**
     * 设置摄像头美颜参数。
     *
     * @param cameraDeviceId 摄像头设备 Id。
     * @param beautyParam 摄像头美颜参数，详情请参见  {@link TXCameraBeautyParam}。
     */
    virtual void setCameraBeautyParam(const char* cameraDeviceId, const TXCameraBeautyParam* beautyParam) = 0;

    /**
     * 启用摄像头采集画面绿幕抠图。
     *
     * @param cameraDeviceId 摄像头设备 Id。
     * @param enable 是否启用摄像头采集画面绿幕抠图。
     * @note 当前仅 Windows 端支持。
     */
    virtual void enableCameraGreenScreen(const char* cameraDeviceId, bool enable) = 0;

    /**
     * 启动屏幕/窗口采集。
     *
     * @param screenSource 屏幕/窗口采集的详细参数，详情请参见  {@link TRTCScreenCaptureSourceInfo}。
     * @param captureRect 指定捕获的区域。
     */
    virtual void startScreenSource(const TRTCScreenCaptureSourceInfo& screenSource, const RECT& captureRect) = 0;

    /**
     * 更新窗口/屏幕采集属性。
     *
     * @param sourceId 屏幕/窗口的 Id。
     * @param property 屏幕/窗口采集参数，详情见 {@link TRTCScreenCaptureProperty}。
     */
    virtual void updateScreenCaptureProperty(TXView sourceId, const liteav::TRTCScreenCaptureProperty& property) = 0;

    /**
     * 停止屏幕/窗口采集。
     *
     * @param screenSourceId 屏幕/窗口的 Id。
     */
    virtual void stopScreenSource(const TXView screenSourceId) = 0;

    /**
     * 启动图片采集。
     *
     * @param imagePath 图片路径 ，目前只支持 BMP、JPG、PNG、GIF 四种格式。
     * @param fps 采集输出帧率，可以不设置由 SDK 做最佳决策。
     */
    virtual void addImageSource(const char* imagePath, int fps = 0) = 0;

    /**
     * 停止图片采集。
     *
     * @param imagePath 图片路径。
     */
    virtual void removeImageSource(const char* imagePath) = 0;

    /**
     * 开启手机投屏。
     *
     * @param phoneMirrorSourceId 手机投屏Id，由用户指定。
     * @param param 手机投屏参数，详情见 {@link TXPhoneMirrorParam}。
     *
     * @note 该接口会开启投屏设备检测功能。
     * @note 此功能默认没有打包到 SDK 中，如果想使用此功能，联系腾讯单独提供 SDK。
     * @note 当前仅 Windows 端支持。
     */
    virtual void startPhoneMirrorSource(const char* phoneMirrorSourceId, const TXPhoneMirrorParam& param) = 0;

    /**
     * 停止手机投屏。
     *
     * @param phoneMirrorSourceId 手机投屏Id，由用户指定。
     *
     * @note 此功能默认没有打包到 SDK 中，如果想使用此功能，联系腾讯单独提供 SDK。
     * @note 当前仅 Windows 端支持。
     */
    virtual void stopPhoneMirrorSource(const char* phoneMirrorSourceId) = 0;

    /**
     * 设置手机投屏参数。
     *
     * @param phoneMirrorSourceId 手机投屏Id，由用户自己指定。
     * @param param               手机投屏配置参数，详情见 {@link TXPhoneMirrorParam }
     *
     * @note 此功能默认没有打包到 SDK 中，如果想使用此功能，联系腾讯单独提供 SDK。
     * @note 当前仅 Windows 端支持。
     */
    virtual void setPhoneMirrorParam(const char* phoneMirrorSourceId, const TXPhoneMirrorParam& param) = 0;

    /**
     * 添加在线视频。
     * @param url 在线视频地址。
     */
    virtual void addOnlineVideoSource(const char* url) = 0;

    /**
     * 设置在线视频播放参数。
     */
    virtual void setOnlineVideoParam(const char* url, const OnlineVideoParam& param) = 0;

    /**
     * 移除在线视频。
     * @param url 在线视频地址。
     */
    virtual void removeOnlineVideoSource(const char* url) = 0;

    /**
     * 添加视频文件。
     * @param videoFilePath 视频文件地址。
     */
    virtual void addVideoFileSource(const char* videoFilePath) = 0;

    /**
     * 设置视频文件播放参数。
     */
    virtual void setVideoFileParam(const char* videoFilePath, const VideoFileParam& param) = 0;

    /**
     * 设置视频文件播放位置。
     */
    virtual void seekVideoFileSource(const char* videoFilePath, const int64_t positionMs) = 0;

    /**
     * 暂停视频文件。
     */
    virtual void pauseVideoFileSource(const char* videoFilePath) = 0;

    /**
     * 继续播放视频文件。
     */
    virtual void resumeVideoFileSource(const char* videoFilePath) = 0;

    /**
     * 移除视频文件。
     */
    virtual void removeVideoFileSource(const char* videoFilePath) = 0;

    /**
     * 设置渲染窗口。
     *
     * @param view 渲染窗口。
     */
    virtual void setMixedVideoRenderView(TXView view) = 0;

    /**
     * 设置混流画面的填充模式。
     *
     * @param fillMode 指定填充模式，详情请参见 {@link TRTCVideoFillMode}。
     */
    virtual void setMixedVideoRenderFillMode(TRTCVideoFillMode fillMode) = 0;

    /**
     * 将当前 ITXLocalMediaTranscoding 附着至 TRTC。
     *
     * @param trtcCloud TRTC 实例指针。
     * @note SDK 会根据 {@link startTranscoding} 设置的 TRTCVideoStreamType 来决定用哪一路流推流。
     */
    virtual void attachTRTC(ITRTCCloud* trtcCloud) = 0;

    /**
     * 将当前 ITXLocalMediaTranscoding 和 TRTC 分离。
     */
    virtual void detachTRTC() = 0;

    /**
     * 调用实验性接口
     */
    virtual const char* callExperimentalAPI(const char* jsonStr) = 0;

   protected:
    virtual ~ITXLocalMediaTranscoding() {
    }
};

}  // namespace liteav

#endif  // TRTC_CPP_ITXLOCALMEDIATRANSCODING_H_

#endif
