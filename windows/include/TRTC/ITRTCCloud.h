 ...
 */
     ...
     * 1.3 添加 TRTC 事件回调。
     *
     * 您可以通过 {@link ITRTCCloudCallback} 获得来自 SDK 的各类事件通知（例如：错误码，警告码，音视频状态参数等）。
     */
     ...
     * </pre>
     *
     * @param param 需要您传入 JSON 格式的字符串参数，`roomId` 代表数字格式的房间号，`strRoomId` 代表字符串格式的房间号，`userId` 代表目标主播的用户 ID，`muteAudio`, `muteVideo`,`muteSubStream`
     * 均为可选项，分别代表禁止或允许跨房主播发布音频/主路视频/辅路视频的能力。
     */
 ...
 */
    ...
    /**
     * 9.1 获取 AI 转录管理类（AITranscriberManager）。
     *
     * 通过 AI 转录管理，您可以启动/停止实时转录服务，对指定用户的音频进行实时转录和翻译。
     * @return AI 转录管理类 {@link AITranscriberManager}。
     */
 ...
 */
     ...
     * @note TRTC 已经内置对媒体流进行加密后再传输，启用媒体流私有加密后将使用您传入的密钥与初始向量进行再次加密。
     */
