...
class V2TXLivePusher {
   public:
     ...
     * 实例下，仅有一个采集源可以上行，不同采集源之间切换，请先关闭前一采集源，再开启后一采集源，保证同一采集源的开启和关闭是成对调用的。例如：采集源从Camera切换到VirtualCamera，调用顺序是 startCamera -> stopCamera -> startVirtualCamera。
     */
     ...
     * 开启清晰度增强。
     */
    virtual void enableSharpnessEnhancement(bool enable) = 0;
     ...
     */
    virtual void setLUTColorFilter(V2TXLiveImage* image) = 0;
     ...
     */
    virtual void setLUTColorFilterStrength(float strength) = 0;
     ...
     *
     * @note startVirtualCamera，startCamera，startScreenCapture，同一 Pusher
     * 实例下，仅有一个采集源可以上行，不同采集源之间切换，请先关闭前一采集源，再开启后一采集源，保证同一采集源的开启和关闭是成对调用的。例如：采集源从Camera切换到ScreenCapture，调用顺序是 startCamera -> stopCamera -> startScreenCapture。
     */
     ...
     *        - path 默认为空，其他值未定义。（ Mac 、Android 平台）。
     * @note 此接口目前仅适用于 Windows 、 Mac 、Android 平台。
     */
     ...
     * @param config 请参考 V2TXLiveDef.hpp 中关于 {@link V2TXLiveTranscodingConfig} 的介绍。如果传入 nullptr 则取消云端混流转码。
     * @return 返回值 {@link V2TXLiveCode}。
     *         - V2TXLIVE_OK: 成功。
     ...
     * - 请放心，您退房时会自动取消混流状态。
     */
    virtual int32_t setMixTranscodingConfig(V2TXLiveTranscodingConfig* config) = 0;
     ...
     */
    virtual int startLocalRecording(const V2TXLiveLocalRecordingParams& params) = 0;
