...
class TestPageState extends State<TestPage> {
  ...
  speedTestListener(type, param) {
    if (type == TRTCCloudListener.onSpeedTest) {
      print("=====onSpeedTest=====");
      var currentResult = param['currentResult'];
      int finishedCount = param['finishedCount'];
      int totalCount = param['totalCount'];

      int quality = currentResult['quality'];
      String ip = currentResult['ip'];
      double upLostRate = currentResult['upLostRate'];
      double downLostRate = currentResult['downLostRate'];
      int rtt = currentResult['rtt'];

      print("TRTCCloudListener onSpeedTest\n"
          "finishedCount: $finishedCount \n"
          "totalCount: $totalCount \n"
          "quality: $quality \n"
          "ip: $ip \n"
          "upLostRate: $upLostRate \n"
          "downLostRate: $downLostRate \n"
          "rtt: $rtt \n");
    } else if (type == TRTCCloudListener.onSpeedTestResult) {
      print("=====onSpeedTestResult=====");
      ...
      double upLostRate = result['upLostRate'];
      double downLostRate = result['downLostRate'];
      int rtt = result['rtt'];
      int availableDownBandwidth = result['availableDownBandwidth'];
      int availableUpBandwidth = result['availableUpBandwidth'];
      int upJitter = result['upJitter'];
      int downJitter = result['downJitter'];
      print("TRTCCloudListener onSpeedTestResult\n"
          "success: $success \n"
          "errMsg: $errMsg \n"
          "ip: $ip \n"
          "quality: $quality \n"
          ...
          "downLostRate: $downLostRate \n"
          "rtt: $rtt \n"
          "availableDownBandwidth: $availableDownBandwidth \n"
        ...
        body: TabBarView(children: [
          ListView(
            children: [
              ...
              TextButton(
                ...
                child: Text('startSpeedTest-deprecated'),
              ),
              TextButton(
                onPressed: () async {
                  TRTCSpeedTestParams params = TRTCSpeedTestParams(
                    ...
                    userId: "123666",
                    userSig:  GenerateTestUserSig.genTestSig("123666"),
                  ...
                  MeetingTool.toast(returnValue.toString(), context);
                },
                child: Text('startSpeedTestWithParams'),
              ),
              TextButton(
                onPressed: () async {
                  trtcCloud.stopSpeedTest();
                  trtcCloud.unRegisterListener(speedTestListener);
                },
          ...
          ListView(
            children: [
              ...
              TextButton(
                  onPressed: () async {
                    var preloadParam = AudioMusicParam(
                        id: 223,
                        publish: true,
                        path: await MeetingTool.copyAssetToLocal(
                            'media/daoxiang.mp3'));
                    txAudioManager.preloadMusic(preloadParam, TXMusicPreloadObserver(
                        onLoadError: (int id, int errCode) {
                          debugPrint("TXMusicPreloadObserver onLoadError id: ${id}, errCode: ${errCode}");
                        },
                        onLoadProgress: (int id, int progress) {
                          debugPrint("TXMusicPreloadObserver onLoadProgress id: ${id}, progress: ${progress}");
                        }
                    ));
                  },
                  child: Text('preloadMusic-local'),
              ),
              TextButton(
                onPressed: () async {
                  var preloadParam = AudioMusicParam(
                      id: 224,
                      publish: true,
                      path: "http://downsc.chinaz.net/Files/DownLoad/sound1/201906/11582.mp3");
                  txAudioManager.preloadMusic(preloadParam, TXMusicPreloadObserver(
                      onLoadError: (int id, int errCode) {
                        debugPrint("TXMusicPreloadObserver onLoadError id: ${id}, errCode: ${errCode}");
                      },
              ...
              TextButton(
                onPressed: () async {
                  var preloadParam = AudioMusicParam(
                      id: 223,
                      publish: true,
                      path: "https://c6.y.qq.com/base/fcgi-bin/u?__=LZIHLOwTPw27");
                  txAudioManager.preloadMusic(preloadParam, TXMusicPreloadObserver(
                      onLoadError: (int id, int errCode) {
                        debugPrint("TXMusicPreloadObserver onLoadError id: ${id}, errCode: ${errCode}");
                      },
          ...
          ListView(children: [
            ...
            TextButton(
              ...
              child: const Text('enbalePictureInPicture'),
            ),
          ]),
          ListView(
            children: [
              ...
              TextButton(
                  onPressed: () async {
                    int? result = await txDeviceManager.enableFollowingDefaultAudioDevice(TRTCCloudDef.TXMediaDeviceTypeMic, true);
                  },
                  child: Text('enableFollowingDefaultAudioDevice-true'),
              ),
              TextButton(
                onPressed: () async {
                  int? result = await txDeviceManager.enableFollowingDefaultAudioDevice(TRTCCloudDef.TXMediaDeviceTypeMic, false);
                },
                child: Text('enableFollowingDefaultAudioDevice-false'),
              ),
              TextButton(
                onPressed: () async {
