...
class MeetingPageState extends State<MeetingPage> with WidgetsBindingObserver {
  ...
  _onRtcListener(type, param) async {
    ...
    if (type == TRTCCloudListener.onUserVoiceVolume) {
      ...
      Map<dynamic, dynamic> userVolumeMap = userVolumes[0];
      String userId = userVolumeMap['userId'];
      int volume = userVolumeMap['volume'];

      print("TRTCCloudListener onUserVoiceVolume totalVolume:$totalVolume userId:$userId volume:$volume");
    }

    if (type == TRTCCloudListener.onSystemAudioLoopbackError) {
      int errCode = param['errCode'];
      print("TRTCCloudListener onSystemAudioLoopbackError errCode:$errCode");
    }
